...
void SDM::loadfile(Dfloat *Var,char *nfile,int ktime){
  ...
  MPI_File_open(MPI_COMM_WORLD,times,MPI_MODE_RDONLY,MPI_INFO_NULL,&fhw);
...
void SDM::ReadBoundaries(int itime,int nt){
   ...
   if (((itime % nsteps) == 0) or (itime == nt)) {
	...
	if (itime == nt){
	  sprintf(filename,"temp/BLX-%d.temp",idx_sub);
	  MPI_File_open(MPI_COMM_SELF,filename,MPI_MODE_RDONLY,MPI_INFO_NULL,&BLX);
	}
	...
	if (itime == nt ){
	  sprintf(filename,"temp/BRX-%d.temp",idx_sub);
	  MPI_File_open(MPI_COMM_SELF,filename,MPI_MODE_RDONLY,MPI_INFO_NULL,&BRX);
	}
	...
	if (itime == nt){
	  sprintf(filename,"temp/BLY-%d.temp",idx_sub);
	  MPI_File_open(MPI_COMM_SELF,filename,MPI_MODE_RDONLY,MPI_INFO_NULL,&BLY);
	}
	...
	if (itime == nt ){
	  sprintf(filename,"temp/BRY-%d.temp",idx_sub);
	  MPI_File_open(MPI_COMM_SELF,filename,MPI_MODE_RDONLY,MPI_INFO_NULL,&BRY);
	}
	...
	if (itime == nt){
	  sprintf(filename,"temp/BLZ-%d.temp",idx_sub);
	  MPI_File_open(MPI_COMM_SELF,filename,MPI_MODE_RDONLY,MPI_INFO_NULL,&BLZ);
	}
